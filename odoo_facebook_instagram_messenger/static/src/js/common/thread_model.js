...
patch(Thread.prototype, {
    _computeDiscussAppCategory() {
        if (this.channel_type === "FbChannels" && this.length > 0) {
            return this.store.discuss.FbChannels;
        } else if (this.channel_type === "InstaChannels" && this.length > 0) {
            return this.store.discuss.InstaChannels;
        } else {
            return super._computeDiscussAppCategory();
        }
    },
     get canUnpin() {
        return this.channel_type === "chat" ||  this.channel_type === "FbChannels" ||  this.channel_type === "InstaChannels" && this.importantCounter === 0;
    }
});
//patch(Thread.prototype, {
//    update(data) {
...
//                if (this.type === "chat" || this.type === "FbChannels") {
...
//            }
//            if ("channelMembers" in data) {
//                if (this.type === "chat" || this.type === "InstaChannels") {
//                    for (const member of this.channelMembers) {
//                        if (
//                            member.persona.notEq(this._store.user) ||
//                            (this.channelMembers.length === 1 &&
...
//     */
//    canUnpin(thread) {
//        return thread.type === "FbChannels" || thread.type === 'InstaChannels' || thread.type === "chat" && this.getCounter(thread) === 0;
//    }
//});
