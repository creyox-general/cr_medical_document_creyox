...
class L10nIlSystem1000Export(models.TransientModel):
    ...
    def _export_header(self):
        return  (f"A{self.env.company.l10n_il_tax_deduction_id}")
    ...
    def _export_footer(self):
        count = len(self.env.context.get("active_ids", []))
        return (f"Z{self.env.company.l10n_il_tax_deduction_id}{count}")
